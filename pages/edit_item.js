...
db.getRows('app', { 'cat': 'settings' }, (succ, rows) => {
  if (succ) {
    selectedItem = rows[0].selectedItem;  // Item id

    // Retrieve the information
    db.getRows('items', { 'id': selectedItem }, (succ, data) => {
      if (succ) {
        // Retrieve the items attributes
        $('#item-name').val(data[0].item_name);
        $('#item-description').val(data[0].item_description);
        $('#unit').val(data[0].item_unit);
        $('#quantity').val(data[0].item_quantity);
        $('#buying-price').val(data[0].buying_price);
        $('#selling-price').val(data[0].selling_price);
      } else {
        console.log('Error retrieving the item.');
      }
    });
  } else {
    return;
...
$('#btn-update-item').on('click', () => {
  ...
  // Updatet the item if all the fields are validated
  // Get inputs
  ...
  db.updateRow('items', { 'id': selectedItem }, item, (succ, msg) => {
    if (succ) {
      $('#err-alert').removeClass('hidden');
      $('#err-alert').addClass('alert-success');
      $('#err-alert').html("Item updated successfully!");
    } else {
      $('#err-alert').removeClass('hidden');
      $('#err-alert').addClass('alert-danger');
      $('#err-alert').html("Error updating item. Please contact the administrator.");
    }
  });
